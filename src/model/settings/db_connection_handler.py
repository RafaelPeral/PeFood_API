...
from src.model.entities.bag import Bag, ProductItem
...
class DBConnectionHandler:
    def __init__(self):
        ...
        self.__test_db_name = "test_db"

        self.__document_models = [
            User,
            Address,
            PaymentFormat,
            Product,
    ...
    async def disconnect_db(self):
        if self.__client:
            await self.__client.drop_database(self.__test_db_name)
            self.__client.close()

    async def test_connect_db(self):
        try:
            self.__client = AsyncIOMotorClient(self.connection_string)
            self.__test_db_connection = self.__client[self.__test_db_name]
            await init_beanie(
                database=self.__test_db_connection,
                document_models=self.__document_models,
            )
        except ServerSelectionTimeoutError as e:
            logger.error(f"Timeout error connecting to MongoDB: {e}")
            raise Exception(
